...
class LINEModel:
    def __init__(self, args):
        ...
        self.embedding = tf.get_variable('target_embedding_%s%s'%(args.uid, args.flag), [args.num_of_nodes, args.embedding_dim],
        ...
        if args.proximity == 'first-order':
            ...
            self.context_embedding = tf.get_variable('context_embedding_%s%s'%(args.uid, args.flag), [args.num_of_nodes, args.embedding_dim],
                                                     initializer=tf.random_uniform_initializer(minval=-1., maxval=1.))
        ...
        self.variables_list=tf.trainable_variables()
        self.gradients_list = tf.gradients(self.loss, self.variables_list)
