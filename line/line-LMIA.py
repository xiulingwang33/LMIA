...
def qda_test(train_edges, train_edges_false, val_edges, val_edges_false, test_edges, test_edges_false, val_preds,
    ...
    result.to_csv("E:/python/banlance/code/{2}/{3}/result/train{0}_{1}.csv".format(ego_user, f, DATASET, METHOD))
    ...
    result.to_csv("E:/python/banlance/code/{2}/{3}/result/val{0}_{1}.csv".format(ego_user, f, DATASET, METHOD))
    ...
    result.to_csv("E:/python/banlance/code/{2}/{3}/result/test{0}_{1}.csv".format(ego_user, f, DATASET, METHOD))
...
for sed in seds:
    ...
    for ego_user in G_EGO_USERS:
        ...
        g_train1 = g_train
        for node in g_train1.nodes():
            for nbr in sorted(g_train1.neighbors(node)):
        ...
        if dp == 1:
            ...
            F = 'dot-line-' + str(ego_user) + '-' + str(dp)+ord+str(sed)

        res_dir = './'


        train_adj = nx.adjacency_matrix(g_train1, weight='weight')
        train_tuple = (train_adj, ft)
        with open('%s/train-%s-adj-feat.pkl' % (res_dir, ego_user), "wb") as f:
            pk.dump(train_tuple, f)
        ...
        graph_file = '%s/train-%s-adj-feat.pkl' % (res_dir, ego_user)
        train_edge_labels, test_edge_labels, emb_matrix, train_sim_matrix, test_sim_matrix, train_edge_embs, test_edge_embs, train_embs_1, train_embs_2, test_embs_1, test_embs_2, train_edges_sampled= LINE.LINE2(g_train, train_test_split,graph_file, DATASET, METHOD,ego_user, F,dp,res_dir,sigma,ord)
        ...
        for comb in combs:
            ...
            report=metrics.classification_report(y_test[:,2], y_score, labels=range(3),output_dict=True)


            out="{}{}-mlp_sim-report_{}.csv".format(res_dir, F,comb)
            ...
            out="{}{}-rf_sim-report_{}.csv".format(res_dir, F,comb)
            df = pd.DataFrame(report).transpose()
            df.to_csv(out, index=True)
            ...
            out="{}{}-svm_sim-report_{}.csv".format(res_dir, F,comb)
            df = pd.DataFrame(report).transpose()
            df.to_csv(out, index=True)
